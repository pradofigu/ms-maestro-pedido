...
{
    public static void OrderCompletedEndpoint(this IRabbitMqBusFactoryConfigurator cfg, IBusRegistrationContext context)
    {
        cfg.ReceiveEndpoint("order-completed-queue", re =>
        {
            ...
            re.ConfigureConsumer<OrderCompleted>(context);

            // the binding of the intermediary exchange and the primary exchange
            re.Bind("order-completed", e =>
            {
